...
FixNEB_MOD::FixNEB_MOD(LAMMPS *lmp, int narg, char **arg) :
  ...
  displacements(NULL),freplicaall(NULL), gradlenall(NULL)
{

  if (narg < 4) error->all(FLERR,"Illegal fix neb_mod command");


  VariableKspring = false;
  AutoKspring = false;
  int iarg = 4;
  if (strcmp(arg[3] , "auto")==0){
    kspring = -1.0;
    AutoKspring = true;}
  else if(strcmp(arg[3], "variable") == 0){
    kspring = -1.0;
    kmax = force->numeric(FLERR, arg[4]);
    kdelta = force->numeric(FLERR, arg[5]);
    iarg = 6;
    VariableKspring = true;
  }else{
    kspring = force->numeric(FLERR,arg[3]);
  }

  if (kspring <= 0.0 && !AutoKspring &&!VariableKspring) error->all(FLERR,"Illegal fix neb_mod command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"parallel") == 0) {
      ...
      if (strcmp(arg[iarg+1],"ideal") == 0) {
        if (VariableKspring) error->all(FLERR, "Cannot use parallel ideal with Variable Spring Constant");
  ...
  if (NEB_MODLongRange || AutoKspring || VariableKspring) {
...
FixNEB_MOD::~FixNEB_MOD()
{
  ...
  if (NEB_MODLongRange || AutoKspring) {
...
void FixNEB_MOD::min_post_force(int vflag)
{
  ...
  double vIni=0.0;
  double emax, eref, buf, eprev, enext; // for variable spring constant

  vprev = vnext = veng = emax = eref =pe->compute_scalar();
  ...
  if (VariableKspring){
    if (me == 0)
      MPI_Allreduce(&veng, &emax, 1, MPI_DOUBLE, MPI_MAX, rootworld);

    if (ireplica == 0)
      MPI_Send(&veng, 1, MPI_DOUBLE, universe->root_proc[nreplica-1], 0, uworld);
    if (ireplica == nreplica-1){
      MPI_Recv(&buf,1,MPI_DOUBLE,universe->root_proc[0],0,uworld,MPI_STATUS_IGNORE);
      if (buf > eref)
        eref = buf;
    }
    if (me ==0){
      MPI_Bcast(&eref, 1, MPI_DOUBLE, nreplica-1, rootworld);
    }
    if (cmode == MULTI_PROC) {
      MPI_Bcast(&eref,1,MPI_DOUBLE,0,world);
      MPI_Bcast(&emax,1,MPI_DOUBLE,0,world);
    }

    eprev = MAX(veng, vprev);
    enext = MAX(veng, vnext);
    if (eprev > eref){
      kprev = kmax - kdelta * (emax - eprev) / (emax - eref);
    }else{
      kprev = kmax - kdelta;
    }
    if (enext > eref){
      knext = kmax - kdelta * (emax - enext) / (emax - eref);
    }else{
      knext = kmax - kdelta;
    }

    //no meaning
    kspring = (kprev + knext)/2.0;
  }
  ...
  // kspring evaluation (automatic update of spring constant)
  double gradlentot = 0;
  if (AutoKspring){
    if (cmode == SINGLE_PROC_DIRECT || cmode == SINGLE_PROC_MAP) {
      MPI_Allgather(&gradlen,1,MPI_DOUBLE,&gradlenall[0],1,MPI_DOUBLE,uworld);
    } else {
      if (me == 0)
        MPI_Allgather(&gradlen,1,MPI_DOUBLE,&gradlenall[0],1,MPI_DOUBLE,rootworld);
      MPI_Bcast(gradlenall,nreplica,MPI_DOUBLE,0,world);
    }
    for (int i = 1; i < nreplica-1; i++)
      gradlentot += gradlenall[i];

    kspring = gradlentot / lentot;
  }
  ...
  if (ireplica == rclimber) prefactor = -2.0*dot;
    ...
    if (NEB_MODLongRange) {
      prefactor = -dot - kspring*(lenuntilIm-idealPos);
    } else if (VariableKspring){
      prefactor = -dot + knext*nlen-kprev*plen;
    }
    ...
    if (FinalAndInterWithRespToEIni&& veng<vIni) {
      ...
      if (NEB_MODLongRange) {
        prefactor =  -kspring*(lenuntilIm-idealPos);
      } else if (VariableKspring){
        prefactor = -dot + knext*nlen-kprev*plen;
      }
      else if (StandardNEB_MOD) {
...
void FixNEB_MOD::reallocate()
{
  ...
  if (AutoKspring) {
    ...
    memory->destroy(gradlenall);
    memory->create(gradlenall, nreplica, "neb_mod:gradlenall");
  }
}
