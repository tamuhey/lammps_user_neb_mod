...
NEB_MOD::NEB_MOD(LAMMPS *lmp) : Pointers(lmp) {}

/* ----------------------------------------------------------------------
   internal NEB_MOD constructor, called from TAD
------------------------------------------------------------------------- */

NEB_MOD::NEB_MOD(LAMMPS *lmp, double etol_in, double ftol_in, int n1steps_in,
...
/* ----------------------------------------------------------------------
   perform NEB_MOD on multiple replicas
------------------------------------------------------------------------- */

void NEB_MOD::command(int narg, char **arg)
{
  if (domain->box_exist == 0)
    error->all(FLERR,"NEB_MOD command before simulation box is defined");

  if (narg < 6) error->universe_all(FLERR,"Illegal NEB_MOD command");
  ...
  if (etol < 0.0) error->all(FLERR,"Illegal NEB_MOD command");
  if (ftol < 0.0) error->all(FLERR,"Illegal NEB_MOD command");
  if (nevery <= 0) error->universe_all(FLERR,"Illegal NEB_MOD command");
  if (n1steps % nevery || n2steps % nevery)
    error->universe_all(FLERR,"Illegal NEB_MOD command");
  ...
  if (nreplica == 1) error->all(FLERR,"Cannot use NEB_MOD with a single replica");
  if (atom->map_style == 0)
    error->all(FLERR,"Cannot use NEB_MOD unless atom map exists");

  // process file-style setting to setup initial configs for all replicas

  if (strcmp(arg[5],"final") == 0) {
    if (narg != 7 && narg !=8) error->universe_all(FLERR,"Illegal NEB_MOD command");
    infile = arg[6];
    readfile(infile,0);
    ...
    if (narg != 6 && narg !=7) error->universe_all(FLERR,"Illegal NEB_MOD command");
  } else error->universe_all(FLERR,"Illegal NEB_MOD command");

  verbose=false;
  if (strcmp(arg[narg-1],"verbose") == 0) verbose=true;
  // run the NEB_MOD calculation

  run();
}

/* ----------------------------------------------------------------------
   run NEB_MOD on multiple replicas
------------------------------------------------------------------------- */
...
void NEB_MOD::run()
{
  ...
  int ineb_mod;
  for (ineb_mod = 0; ineb_mod < modify->nfix; ineb_mod++)
    if (strcmp(modify->fix[ineb_mod]->style,"neb_mod") == 0) break;
  if (ineb_mod == modify->nfix) error->all(FLERR,"NEB_MOD requires use of fix neb_mod");

  fneb_mod = (FixNEB_MOD *) modify->fix[ineb_mod];
  if (verbose) numall =8;
  else  numall = 4;
  memory->create(all,nreplica,numall,"neb_mod:all");
  ...
  if (update->minimize->searchflag)
    error->all(FLERR,"NEB_MOD requires damped dynamics minimizer");

  // setup regular NEB_MOD minimization
  FILE *uscreen = universe->uscreen;
  FILE *ulogfile = universe->ulogfile;

    if (me_universe == 0 && uscreen)
      fprintf(uscreen,"Setting up regular NEB_MOD ...\n");
    ...
    if (update->laststep < 0)
      error->all(FLERR,"Too many timesteps for NEB_MOD");
      ...
      if (uscreen) {
        if (verbose) {
          fprintf(uscreen,"Step MaxReplicaForce MaxAtomForce "
                  "GradV0 GradV1 GradVc EBF EBR RDT Kspring RD1 PE1 RD2 PE2 ... "
    ...
    // perform regular NEB_MOD for n1steps or until replicas converge
    // retrieve PE values from fix NEB_MOD and print every nevery iterations
  ...
  // switch fix NEB_MOD to climbing mode if n2steps > 0
  ...
  if (n2steps > 0){
    ...
    // setup climbing NEB_MOD minimization
    ...
    if (me_universe == 0) {
      if (uscreen)
        if (verbose) {
          fprintf(uscreen,"Step MaxReplicaForce MaxAtomForce "
                  "GradV0 GradV1 GradVc EBF EBR RDT Kspring "
    ...
    // perform climbing NEB_MOD for n2steps or until replicas converge
    // retrieve PE values from fix NEB_MOD and print every nevery iterations
...
void NEB_MOD::readfile(char *file, int flag)
{
  ...
  if (me_universe == 0 && screen)
    fprintf(screen,"Reading NEB_MOD coordinate file(s) ...\n");
...
/* ----------------------------------------------------------------------
   universe proc 0 opens NEB_MOD data file
   test if gzipped
------------------------------------------------------------------------- */

void NEB_MOD::open(char *file)
{
  compressed = 0;
...
/* ----------------------------------------------------------------------
   query fix NEB_MOD for info on each replica
   universe proc 0 prints current NEB_MOD status
------------------------------------------------------------------------- */

void NEB_MOD::print_status()
{
  ...
  one[1] = fneb_mod->plen;
  one[2] = fneb_mod->nlen;
  one[3] = fneb_mod->gradlen;

  if (verbose) {
    one[4] = fneb_mod->dotpath;
    one[5] = fneb_mod->dottangrad;
    one[6] = fneb_mod->dotgrad;
    one[7] = fneb_mod->kspring;
  }
  ...
  // take them straight from fix_neb_mod
  ...
  if (me_universe == 0) {
    ...
    FILE *ulogfile = universe->ulogfile;
    if (uscreen) {
      fprintf(uscreen,BIGINT_FORMAT " %24.20g %24.20g ",
              update->ntimestep,fmaxreplica,fmaxatom);
      fprintf(uscreen,"%24.20g %24.20g %24.20g ",
              gradvnorm0,gradvnorm1,gradvnormc);
      fprintf(uscreen,"%24.20g %24.20g %24.20g ",ebf,ebr,endpt);
      if (verbose) { 
        fprintf(uscreen,"%24.20g ", one[7]);}
                
      for (int i = 0; i < nreplica; i++)
        fprintf(uscreen,"%24.20g %24.20g ",rdist[i],all[i][0]);
      if (verbose) {
    ...
    if (ulogfile) {
      fprintf(ulogfile,BIGINT_FORMAT " %24.20g %24.20g ",
              update->ntimestep,fmaxreplica,fmaxatom);
      fprintf(ulogfile,"%24.20g %24.20g %24.20g ",
              gradvnorm0,gradvnorm1,gradvnormc);
      fprintf(ulogfile,"%24.20g %24.20g %24.20g ",ebf,ebr,endpt);
      if (verbose) { 
        fprintf(ulogfile, "%24.20g ", one[7]);}

      for (int i = 0; i < nreplica; i++)
        fprintf(ulogfile,"%24.20g %24.20g ",rdist[i],all[i][0]);
      if (verbose) {
