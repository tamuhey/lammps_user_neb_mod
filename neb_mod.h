...
/* ----------------------------------------------------------------------
   Modified by: Yohei Tamura (Sakai Izumi Lab.)
---------------------------------------------------------------------- */

#ifdef COMMAND_CLASS

CommandStyle(neb_mod,NEB_MOD)

#else

#ifndef LMP_NEB_MOD_H
#define LMP_NEB_MOD_H
...
class NEB_MOD : protected Pointers {
 public:
  NEB_MOD(class LAMMPS *);
  NEB_MOD(class LAMMPS *, double, double, int, int, int, double *, double *);
  ~NEB_MOD();
  void command(int, char **);  // process neb command
  void run();                  // run NEB_MOD
